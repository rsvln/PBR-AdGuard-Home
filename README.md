...
Этот сервис решает проблему когда Policy-Based Routing (PBR) не может автоматически заполнять nft sets при использовании redirect правил DNS к AdGuard Home. Сервис мониторит Query Log AdGuard Home через API и автоматически добавляет IP адреса доменов в соответствующие nft sets для корректной работы PBR.
