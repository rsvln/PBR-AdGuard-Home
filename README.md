...
- **Прогрев доменов через браузер** (опционально) - решает проблему пустых nft sets после перезагрузки PBR
...
- `BROWSERLESS_ENABLED` - включить прогрев доменов через браузер (`true`/`false`, по умолчанию: `false`)
- `BROWSERLESS_URL` - URL Browserless сервиса (по умолчанию: `http://localhost:3000`)
- `BROWSERLESS_WAIT_TIME` - время ожидания загрузки страницы в мс (по умолчанию: `3000`)
...
Автоматический прогрев доменов через headless браузер (Browserless). После перезагрузки PBR сервис открывает все домены из активных политик в браузере, что вызывает DNS запросы и заполняет nftables sets.

### Установка Browserless (Docker)

На вашем роутере NanoPi R5S с 4GB RAM можно запустить Browserless в Docker:
...
```bash
export BROWSERLESS_ENABLED="true"
export BROWSERLESS_URL="http://localhost:3000"
export BROWSERLESS_WAIT_TIME="3000"  # 3 секунды на загрузку страницы
```
...
4. Для каждого домена открывает страницу через Browserless
5. Браузер загружает страницу → делает DNS запросы
6. AdGuard Home резолвит домены → запросы попадают в Query Log
7. Сервис извлекает IP из Query Log → добавляет в nft sets
8. Маршрутизация работает сразу после перезагрузки

**Примечание:** Если Browserless не настроен (`BROWSERLESS_ENABLED=false`), сервис работает в обычном режиме - IP добавляются по мере реальных запросов от пользователей.
